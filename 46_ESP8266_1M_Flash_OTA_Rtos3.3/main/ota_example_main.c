...
static void ota_example_task(void *pvParameter)
{
    ...
    */
    ...
    ESP_LOGW(TAG, "Connect to http server send :\n%s",http_request);
    if (get_len < 0) {
...
void app_main()
{
    ...
    ESP_LOGW(TAG, "EXAMPLE_WIFI_SSID  %s", EXAMPLE_WIFI_SSID);
    ESP_LOGW(TAG, "EXAMPLE_WIFI_PASS  %s", EXAMPLE_WIFI_PASS);
    ESP_LOGW(TAG, "EXAMPLE_SERVER_IP  %s", EXAMPLE_SERVER_IP);
    ESP_LOGW(TAG, "EXAMPLE_SERVER_PORT  %s", EXAMPLE_SERVER_PORT);
    ESP_LOGW(TAG, "EXAMPLE_FILENAME  %s", EXAMPLE_FILENAME);
