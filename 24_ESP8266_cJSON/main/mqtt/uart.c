...
funUartBack callBack;

void funUart_ReadBack(funUartBack tempCallBack) {
...
LOCAL void uart0_rx_intr_handler(void *para) {
	...
	//1
	uart_rx_intr_disable(UART0);  //�ر��ж�
	...
	uint8 d_tmp = 0;
	uint8 idx = 0;
	uint8 cache[fifo_len];  //�½�һ����������

	//3.ԭ�����
	for (idx = 0; idx < fifo_len; idx++) {
		d_tmp = READ_PERI_REG(UART_FIFO(UART0)) & 0xFF;
		cache[idx] = d_tmp;  //�����յ���ֵ�洢������
	}

	//���ﴦ���Լ��Ĵ���
	if (cache[0] == 240 && cache[fifo_len - 1] == 3) {

		//�����ϱ����ݻ�����������
		if (cache[1] == 1 || cache[1] == 170) {

			//����У���
			uint8 numberTotal = 0x00;
			int j;
			for (j = 1; j < fifo_len - 2; j++) {
				numberTotal = numberTotal + cache[j];
			}
			//����ʶ����У���ӵ������
			numberTotal = numberTotal + 0xA1;

			//uart_tx_one_char(UART0, numberTotal);
			//os_delay_us(60000);
			//uart_tx_one_char(UART0, cache[fifo_len - 2]);
			//os_delay_us(60000);

			//����У���
			if (numberTotal == cache[fifo_len - 2]) {
				//�ϱ��ƾ�״̬:�˿̵�typeΪ0��1
				if (cache[1] == 1) {
					if (cache[2] == 7 && fifo_len == 12) {
						//��3���ֽ�Ϊ���صƣ���4��Ϊcw����5��Ϊww����6��Ϊ�龰ģʽ
						callBack(cache[3], cache[4], cache[5], cache[6]);
					} else {
						//���ݳ��ȴ���ص�
						u8 error[] = { 0xF1, 0xFF, 0x01, 0x02, 0x00, 0x03 };
						error[4] = error[1] + error[2] + error[3];
						int i = 0;
						for (i = 0; i < 6; i++) {
							uart_tx_one_char(UART0, error[i]);
						}
					}
				}

				//�������󣬴˿̵�typeΪ3
				//����ǰ�豸���ͣ��˿̵�typeΪ2�����ص���     1.cw ww   2.rgb   3.rgb +cw ww��
				if (cache[1] == 170 && cache[2] == 1) {
					if (cache[3] == 1) {
						callBack(3, cache[4], cache[5], cache[6]);
					} else if (cache[3] == 2) {
						callBack(4, cache[4], cache[5], cache[6]);
					} else {
						//���ݳ��ȴ���ص�
						u8 error[] = { 0xF1, 0xFF, 0x01, 0x02, 0x00, 0x03 };
						error[4] = error[1] + error[2] + error[3];
				...
				//У��ʹ���ص�
				u8 error[] = { 0xF1, 0xFF, 0x01, 0x01, 0x00, 0x03 };
				error[4] = error[1] + error[2] + error[3];
				int i = 0;
				for (i = 0; i < 6; i++) {
					uart_tx_one_char(UART0, error[i]);
...
uart_recvTask(os_event_t *events) {
	if (events->sig == 0) {
		...
		if (cache[0] == 'a' && cache[1] == 'b') { //���ﴦ���Լ��Ĵ���
			os_printf("finally found you\r\n");
		} else {
			os_printf("finally not sfound you\r\n");
		}
