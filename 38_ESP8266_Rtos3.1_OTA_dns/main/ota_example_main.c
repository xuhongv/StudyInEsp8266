...
static int connect_to_http_server()
{
    ...
    for (cur = addr_list; cur != NULL; cur = cur->ai_next)
    {
        ...
        serverAddr.sin_port = htons(EXAMPLE_SERVER_PORT); // 默认以8080端口连接
...
static void ota_example_task(void *pvParameter)
{
    ...
    */
    ...
    ESP_LOGW(TAG, "http_request buffer: %s", http_request);
    ...
    if (err != ESP_OK)
    {
        ESP_LOGW(TAG, "esp_ota_begin failed, error=%d", err);
        task_fatal_error();
    }
...
void app_main()
{
    ...
    initialise_wifi();
    xTaskCreate(&ota_example_task, "ota_example_task", 1024 * 9, NULL, 8, NULL);
}
